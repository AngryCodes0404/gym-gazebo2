...
def getExclusiveNetworkParameters():
    ...
    file.write(
        ...
        + "\nROS_DOMAIN_ID="
        + str(randomPortROS)
        + "\nGAZEBO_MASTER_URI=http://localhost:"
        + str(randomPortGazebo)
    )
    file.close()
...
def generateLaunchDescriptionMara(gzclient, realSpeed, multiInstance, port, urdf):
    ...
    if realSpeed:
        worldPath = os.path.join(
            os.path.dirname(gym_gazebo2.__file__), "worlds", "empty.world"
        )
    else:
        worldPath = os.path.join(
            os.path.dirname(gym_gazebo2.__file__), "worlds", "empty_speed_up.world"
        )
    ...
    launchDesc = LaunchDescription(
        [
            ...
            Node(
                package="mara_utils_scripts",
                node_executable="spawn_mara.py",
                arguments=[urdf],
                output="screen",
            ),
            Node(
                package="hros_cognition_mara_components",
                node_executable="hros_cognition_mara_components",
                output="screen",
                arguments=[
                    "-motors",
                    installDir + "/share/hros_cognition_mara_components/motors.yaml",
                    "sim",
                ],
            ),
            Node(
                package="mara_contact_publisher",
                node_executable="mara_contact_publisher",
                output="screen",
            ),
        ]
    )
